...
// ─── column-count thresholds matching Tailwind breakpoints ───────────────────
...
const CARD_ASPECT = 3 / 2;   // poster is 2:3 → height = cardWidth * (3/2)
const CARD_BOTTOM_INFO = 80; // px reserved below the image (title + year area)
...
// ─── Virtualized Grid ────────────────────────────────────────────────────────
function VirtualizedGrid({ items, likedItems, onLike, onShowDetails, img_300, img_500, imagenotfound }) {
    return (
        // AutoSizer fills available width; height is constrained to viewport
        <div style={{ height: '75vh', width: '100%' }}>
            <AutoSizer>
                {({ width, height }) => {
                    const cols = getColumnCount(width);
                    const cardWidth = (width - CARD_GAP * (cols - 1)) / cols;
                    const cardHeight = cardWidth * CARD_ASPECT + CARD_BOTTOM_INFO;
                    ...
                    function rowRenderer({ index, key, style }) {
                        ...
                        return (
                            <div key={key} style={{ ...style, display: 'flex', gap: CARD_GAP }}>
                                {row.map((item, colIdx) => (
                                    <div
                                        key={`${item.id}-${item.media_type}-${colIdx}`}
...
function ContentCard({ item, isLiked, onLike, onShowDetails, img_300, img_500, imagenotfound }) {
    ...
    return (
        <motion.div
            ...
            className="bg-gray-800/50 backdrop-blur-sm rounded-xl overflow-hidden border border-gray-700 hover:border-gray-600 transition-all duration-300 cursor-pointer group h-full"
